...
begin
  ...
  vCentraliza := False;   //Não pode ser utilizado. Não está pronto o Finaliza Pedido
  ...
  if (DM.Configuracao1.Empresa = empEletro) and
     (DM.Configuracao1.CodigoCNC = 30) then  //Mossoró
  Begin
...
begin
  ...
  Begin
    ...
    End;
    if IsEmpty and
       (Length(edtCodigoPRD.Text) > 5) then //Referências maiores que 3 caracteres
    Begin
    ...
    Begin
      Showmessage('Produto sem preço de venda!');
      edtCodigoPRD.SetFocus;
      Exit;
    End;
  end;
  ...
  Begin
    ...
    Begin
      ...
      ShowMessage('Produto já inserido, corrija a quantidade!');
    end
    else //Venda com código de Barras ItemAutomático
    Begin
...
begin
  ...
  Begin
    ...
    Begin
      ...
      Else If (DM.Configuracao1.CodigoCNC=DM.Usuario1.CentroDeCusto) and //Se for de outro centro não pode ser salvo
...
begin
  ...
  if Item_Pedido_Venda1.Entrega then
    ...
    Item_Pedido_Venda1.FieldByName('Entrega').asString:='Não';
...
begin
  ...
  Begin
    ...
    Begin
      ...
      Open;
      if IsEmpty then
        Raise exception.Create('Produto não encontrado! '+edtCodigoPRD.Text);
    end;
    ...
    Begin
      ...
      if DM.qr_consultas2.IsEmpty or
         ((not AgenteFin1.RecebimentoAVista) and
          (DM.qr_consultas2.FieldByName('TBP_PRAZO_MEDIO').asInteger < iQtdParcelas)) then //Qtd máxima de parcelas
      Begin
        edtCodigoTBP.Text := '0';
        ShowMessage('Tabela inválida ou vencida!');
      end;
...
begin
  ...
  Begin
    if not Inserir(DM.Configuracao1.CodigoCNC,0,0,0,0,0,
       ...
       DM.Configuracao1.DataHora,0(*DataPrevisão*)) then
...
begin
  ...
  If not Entidade.IsEmpty Then
  Begin
    If (DM.Configuracao1.CodigoCNC=DM.Usuario1.CentroDeCusto) and //Se for de outro centro não pode ser salvo
       ((Entidade.Situacao<>3) and (Entidade.Situacao<>4) and (Entidade.Situacao<>6)) then
      Raise Exception.Create('Salve ou cancele o pedido antes de continuar')
    Else If (DM.Configuracao1.CodigoCNC=DM.Usuario1.CentroDeCusto) and //Se for de outro centro não pode ser salvo
...
begin
  ...
  Begin
    if DM.Configuracao1.CodigoCNC = 11 then
      Raise exception.Create('No depósito esta opção não pode ser marcada!');
    iEntregaDeposito := 1;
  end
  else
    iEntregaDeposito := 0;
  ...
  Begin
    ShowMessage('Tipo de Pagamento inválido para o cliente! (Boleto)!');
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        ShowMessage('Forma de Pagamento inválida para o cliente! ('+DM.QR_Consultas.FieldByName('FPG_DESCRICAO').asString+')');
        If (DM.Usuario1.Permissoes.IndexOf(IntToStr(6750)) < 0) then
        Begin
          Application.CreateForm(TDigitaSenha, DigitaSenha);
          With DigitaSenha do
        ...
        Begin
          ...
          Begin
            ...
            Begin
              ...
              begin
                free;
                raise Exception.Create('O usuário não tem permissão alterar Venda A Vista para Venda A Prazo!');
              end
              else
...
begin
  ...
  if Item_Pedido_Venda1.RecordCount = 1 then
    Raise exception.Create('Não é permitido excluir o último item!'+#13+
                           'Cancele o pedido!');
...
begin
  ...
  if (not ckbEntregue.Checked) then
    if Cliente1.IsEmpty then
      ...
      mensagem:=mensagem+'Para entrega posterior é necessário selecionar o cliente!'+#13;
  if (DM.Configuracao1.Empresa <> empHOPE) and
     (edtBonificacao.Text <> '0') and
     (StrToInt(edtCodigoTBP.Text)=0) then
    mensagem:=mensagem+'A bonificação somente é permitida quando especificada na tabela de preço!'+#13;
  ...
  if (StrToFloat(edtQuantidade.Text) <= 0) or
    ...
    raise Exception.Create('Quantidade/Bonificação digitada incorretamente!');

  if StrToFloat(edtBonificacao.Text) > StrToFloat(edtQuantidade.Text) then
    raise Exception.Create('Bonificação superior a quantidade entregue!');

  if DM.Configuracao1.Empresa = empEletro then //Eletro Guerra
  Begin
    if not ckbEntregue.Checked then
      If MessageDlg('Atenção!!!'+#13+
                    'Essa mercadoria será entregue pelo Depósito?',mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      Begin
  ...
  //Petromax não permite desconto quando o cliente tem desconto
  ...
  if (DM.Configuracao1.Empresa = empHope) and
     (Produto1.ForaDeLinha) then
  Begin
    edtCodigoTBP.Text := '0'; //Não é permitido o usu de tabela de preço para produtos Fora de Linha
    vLimSimples       := 0;
    vLimEspecial      := 0;
    vLimSimplesPrazo  := 0;
    vLimEspecialPrazo := 0;
  ...
  Begin
    ...
    Begin
      ...
      if DM.qr_consultas2.IsEmpty or
         ((not AgenteFin1.RecebimentoAVista) and
          (DM.qr_consultas2.FieldByName('TBP_PRAZO_MEDIO').asInteger < iQtdParcelas)) then //Qtd máxima de parcelas
        Raise exception.Create('Tabela inválida ou Vencida!');
      if (DM.qr_consultas2.FieldByName('TBP_AGF_EXCLUSIVO').asInteger > 0) and
         (dblCodigoAGF.Text <> '') then
      Begin
        if (DM.qr_consultas2.FieldByName('TBP_AGF_EXCLUSIVO').asInteger <> dblCodigoAGF.KeyValue) then
  ...
  Begin
    ...
    if (Forma_de_Pagamento1.Prazo = '0') or
       ...
       AgenteFin1.RecebimentoAVista then //Usando Financeira trabalhar com preço de A Vista
    Begin
      ...
      Begin
        //Não exibir mensagem para desconto simples
        if (DM.Usuario1.Permissoes.IndexOf(IntToStr(3540)) < 0) AND //Desconto Total
        ...
        Begin
          ...
          Begin
            ...
            Begin
              ...
              begin
                free;
                raise Exception.Create('Usuário não tem Permissão para Liberar Desconto (Simples)!');
              end
              else
              Begin
                if DigCdPEN > 0 then
    ...
    else //À PRAZO com Desconto e Não é Financeira
    Begin
      ...
      Begin
        if MessageDlg('Produto: '+Produto1.Descricao+#13+
                      'Desconto atribuído (Simples-Prazo)!'+#13+
                      'Deseja continuar?',mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
          Exit;
        ...
        Begin
          ...
          Begin
            ...
            Begin
              ...
              begin
                free;
                raise Exception.Create('O usuário não tem permissão para liberar o desconto (Simples-Prazo)!');
              end
              else
              Begin
                if DigCdPEN > 0 then
  ...
  Begin
    ...
    if (DM.Configuracao1.Empresa = empEletro) and
       ...
       (SComercial = 0) then
    Begin
      if MessageDlg('Código antigo do produto.'+#13+
                    'Deseja continuar!',mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
        exit;
      ...
      Begin
        ...
        Begin
          ...
          Begin
            ...
            begin
              free;
              raise Exception.Create('O usuário não tem permissão Venda com Código do Produto antigo!');
            end
            else
...
begin
  ...
  Begin
    ...
    Begin
      ShowMessage('Pedido em situação não esperada!'+#13+
  ...
  if (Trunc(Entidade.DataHora)<Trunc(DM.Configuracao1.Data)) and
    ...
    Raise exception.Create('Renove o Cabeçalho!'+#13+
    ...
    try
      ...
      Begin
        ...
        Begin
          ...
          else if (Entidade.TipoPag = 5) and (Entidade.CodigoAGF > 5) then //Dinheiro/Depósito
        ...
        Begin
          ...
          if VAVista > 0 then
            ...
            Titulo_receber1.GerarTitPrazo(Entidade.ClienteCNC, Entidade.CodigoCLI,
              ''(*CartãoCV*), VAPrazo+Arredonda(AgenteFin1.TaxaADM) (*Total + TAC*), Entra, TemEntrada, cCartao, ObsTrc, 'TRT_CODIGO',
...
begin
  ...
  Begin
    ...
    Begin
      ShowMessage('Esta venda não será permitida!'+#13+
                  'Procure a gerência!');
      Exit;
    end
    else
    Begin
      ...
      Begin
        ShowMessage('Será necessário autorização para efetuar essa venda!'+#13+
                    'Procure a gerência!');
        Exit;
      end
      else
      Begin
        ...
        Begin
          ...
          if Proibido then
          Begin
            ShowMessage('Esta venda não será permitida (PROIB)!'+#13+
                        'Procure a gerência!');
            Exit;
          end
...
{  Lib3790:=False; //Liberar Venda Cliente Inadiplente
  ...
  //Impedir venda com entrada menor que as prestações para clientes sem histórico de compra
  if (DM.Configuracao1.Empresa = empEletro) and
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        Begin
          //Verificar se a entrada é menor que as prestações
    ...
    Begin
      if MessageDlg('Total de compras à prazo do cliente é inferior a R$ 50,00!'+#13+
                    'A venda foi feita sem entrada ou entrada menor que o valor das prestações!'+#13+
                    'Deseja Continuar?',mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
        Exit;
      ...
      Begin
        ...
        Begin
          ...
          Begin
            ...
            begin
              free;
              raise Exception.Create('O usuário não tem permissão Entrada menor que a prestação!');
            end
            else
  ...
  //Verificar permissão para finalizar Venda à Vista (Petromax)
  {if (DM.Configuracao1.Empresa = empPetromax) and
     (dblFormaPagamento.KeyValue = 1) and
     (DM.Usuario1.Permissoes.IndexOf(IntToStr(6000)) < 0) then
  Begin
    ShowMessage('Usuário não tem permissão para Finalizar à Vista!');
    Exit;
  end;}
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          begin
            free;
            raise Exception.Create('O usuário não tem permissão para vender a Cliente Bloqueado!');
          end
          else
  ...
  Begin
    ...
    if iOpcaoFin in [0,3] then  //Normal ou Pré-Paga
      iGerarTitulos := 1
    ...
    Begin
      ...
      Titulo_receber5.Open;
      if not Titulo_receber5.IsEmpty then
        Raise exception.Create('Convênio inadiplente!');
      ...
      Begin
        ...
        SQL.Text := 'SELECT SUM(ROUND(IPV_QUANTIDADE*IPV_PRECO_DESC,2)) AS TOTAL '+
          ...
          ' AND P1.FPE_CODIGO=0 '+       //Não processado
        ...
        if Arredonda(Informacoes1.LimiteCreditoConvenio) < Arredonda(FieldByName('TOTAL').asFloat+Entidade.Total) then
          Raise exception.Create('Verificar Limite de Crédito de convênio!');
      end;
    end;
  end;
  ...
  sCheques := '';
  if (DM.Configuracao1.Empresa = empEletro) and
     (DM.Usuario1.Mafalda <> 1) and       //Não é Mafalda
     (dblTipoPagamento.KeyValue = 6) then //Cheque
  Begin
    iQtdTRC := Titulo_receber1.QtdeTitulos(Forma_de_Pagamento1.Prazo);
    while True do
    Begin
       sCheques := Trim(InputBox('Entre com a Numeração dos Cheques', 'Cheques (Ex:003229;008765):', ''));
       ...
       if MessageDlg('Numeração incorreta!'+#13+
                     'Deseja desistir da Finalização?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
       Begin
        ...
        Begin
          ...
          Begin
            ...
            begin
              ...
              SB_Informacoes.Enabled     := True;
              SB_FinalizarPedido.Enabled := True;
              raise Exception.Create('O usuário não tem permissão para vender grupo especial em mais de 8 parcelas!');
            end
            else
...
{  if dblTipoPagamento.KeyValue = 13 then //Cartao
  ...
  CdArquiteto := 0;
  //Sistema de Comissão Mudou.
{  if (DM.Configuracao1.Empresa = empEletro) and
     (DM.Usuario1.Mafalda <> 1) then
    if MessageDlg('Vai ter Comissão para Montador?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      try
        CdArquiteto := StrToInt(InputBox('Serviço de Montagem', 'Código do Montador:', ''));
      except
        CdArquiteto := 0;
        ShowMessage('Código do Montador inválido!');
      end;
}
    ...
    try
          ...
          Begin
            ...
            Begin
              if not Titulo_receber3.Inserir(DM.Configuracao1.CodigoCNC,0,
                 ...
                 'Cartão de Crédito-AUTO '+FieldByName('TRC_OBSERVACAO').AsString,
                 ...
                 0(*Crédito*),FieldByName('AGF_CODIGO').AsInteger,
              ...
              if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,FieldByName('CCX_CODIGO').AsInteger,
                     ...
                     'Recebimento do título -AUTO- '+IntToStr(Titulo_receber3.SCodigo),
                     '1' (*Flag*),''(*Numero*),'',
            ...
            Begin
              ...
              Begin
                ...
                if vValorPagar < 0 then
                  vValorPagar := 0;
                If MessageDlg('O Cliente possui R$ '+format('%.2f',[Cliente1.Credito])+' de Crédito!'+#13+#13+
                  'O Cliente terá que pagar R$ '+format('%.2f',[vValorPagar])+#13+#13+
                  'Deseja utilizá-lo?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
                Begin
              ...
              if not Titulo_receber3.Inserir(DM.Configuracao1.CodigoCNC,0,
                 ...
                 0(*DtVerificado*),'À VISTA-AUTO '+FieldByName('TRC_OBSERVACAO').AsString,
                 PreencheZero(2,SEQ),''(*Numero*),sCodigoCARTAO,
              ...
              Begin
                if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,FieldByName('CCX_CODIGO').AsInteger,
                       ...
                       FieldByName('TRC_TIPO_PAG').AsInteger,FieldByName('TRC_TIPO_DOC').AsInteger,
                       FieldByName('CNC_CODIGO').AsInteger,0 (*CdCHQCNC*),0 (*CdCHQ*),
                       'Recebimento do título -AUTO- '+IntToStr(Titulo_receber3.SCodigo)+sDescCredito,
                       '1' (*Flag*),''(*Numero*),''(*OBS*),
            ...
            else if (SEQ = 1) and   //Entrada da Venda com cartão
            ...
            Begin
              if not Titulo_receber3.Inserir(DM.Configuracao1.CodigoCNC,0,
                 ...
                 0(*CréditoCLI*),FieldByName('AGF_CODIGO').AsInteger,
      ...
      Begin
        ...
        Begin
          ...
          //Não movimentar Estoque na Eletro Guerra se for para embarcar
          ...
          //Gerar movimento de estoque físico e baixa estoque físico
          //para aqueles que forem de entrega imediata e
          //o Local de estoque for o próprio centro de custo
          ...
          Else
          Begin
            //Gerar os itens de embarque dos que não são de entrega imediata
            //e se o local de estoque não é o centro de custo atual
            ...
            //Eletro Guerra Não Gerar Movimento, somente tirar da reserva (se for o caso)
            if (DM.Configuracao1.Empresa = empEletro) and
               (DM.Usuario1.Mafalda <> 1) then
            Begin
              if edtDiasOrcamento.Text = '0' then
...
begin
  ...
  if Entidade.EntregaDeposito and
     (DM.Configuracao1.CodigoCNC <> 11) then //Diferente do Depósito
    Raise exception.Create('O Faturamento desse pedido será feito no Depósito!');
  ...
  Begin
    ...
    Begin
      if self.Cliente1.PessoaFisica then //Pessoa Física sempre Cupom Primeiro
      Begin
      ...
      else if Trim(self.Cliente1.CGFRG) <> '' then  //Pessoa Jurídica com CGF Somente Nota
      Begin
        rdgNota.Checked := True;
        rdgCupom.Enabled := False;
      end;
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        vPrecoUnit     := Arredonda(PrecoVenda); //Preço Unitário
      ...
      if (DM.Configuracao1.Empresa = empHope) and
         (Produto1.ForaDeLinha) then
      Begin
        CdTBP := 0; //Não é permitido o usu de tabela de preço para produtos Fora de Linha
        vPrecoSug := Arredonda(vPrecoSug*0.5); //Desconto de 50% Fora de Linha
        vLimSimples       := 0;
        vLimEspecial      := 0;
        vLimSimplesPrazo  := 0;
      ...
      Begin
        ...
        Begin
          ...
          if DM.qr_consultas2.IsEmpty or
             ((not AgenteFin1.RecebimentoAVista) and
              (DM.qr_consultas2.FieldByName('TBP_PRAZO_MEDIO').asInteger < iQtdParcelas)) then //Qtd máxima de parcelas
            ShowMessage('Um dos Itens está vinculado a uma Tabela inválida ou que passou da Validade!')
          else
          Begin
            if (DM.qr_consultas2.FieldByName('TBP_AGF_EXCLUSIVO').asInteger > 0) and
