object fMxLocalizaUsuario: TfMxLocalizaUsuario
  Left = 276
  Top = 190
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Usu'#225'rios do sistema'
  ClientHeight = 500
  ClientWidth = 819
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 59
    Width = 819
    Height = 401
    Align = alClient
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 1
    object DBGrid1: TDBGrid
      Left = 1
      Top = 1
      Width = 817
      Height = 399
      Align = alClient
      DataSource = DSUsuario1
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDrawColumnCell = DBGrid1DrawColumnCell
      Columns = <
        item
          Expanded = False
          FieldName = 'CNC_CODIGO'
          Title.Caption = 'CNC'
          Width = 29
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USU_CODIGO'
          Title.Caption = 'C'#243'digo'
          Width = 51
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USU_NOME'
          Title.Caption = 'Nome'
          Width = 147
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Situacao'
          Width = 78
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'Maquina'
          Title.Caption = 'M'#225'quina'
          Width = 50
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'Perfil'
          Width = 32
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USU_DT_CADASTRO'
          Title.Caption = 'Cadastro'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USU_OBSERVACAO'
          Title.Caption = 'Observa'#231#227'o'
          Width = 290
          Visible = True
        end>
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 460
    Width = 819
    Height = 40
    Align = alBottom
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 2
    DesignSize = (
      819
      40)
    object SB_Imprimir: TSpeedButton
      Tag = 6950
      Left = 625
      Top = 7
      Width = 90
      Height = 26
      Hint = 'Imprimir Etiqueta'
      Anchors = [akTop, akRight]
      Caption = '&Imprimir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        A9A9FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      OnClick = SB_ImprimirClick
    end
    object SpeedButton2: TSpeedButton
      Left = 714
      Top = 7
      Width = 90
      Height = 26
      Hint = 'Cancelar Compra'
      Caption = 'Fechar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        929249FDFDFDFDFDFDFDFDFDFDFDFDFDF7F7A4FDFDFDFDFDFDFD}
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton2Click
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 819
    Height = 59
    Align = alTop
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 0
    object RadioGroup1: TRadioGroup
      Left = 10
      Top = 9
      Width = 185
      Height = 40
      Caption = '  Ordenar Por  '
      Columns = 2
      ItemIndex = 1
      Items.Strings = (
        'C'#243'digo'
        'Nome')
      TabOrder = 0
      OnClick = RadioGroup1Click
    end
    object RadioGroup2: TRadioGroup
      Left = 205
      Top = 9
      Width = 185
      Height = 40
      Caption = '  Ordem  '
      Columns = 2
      ItemIndex = 1
      Items.Strings = (
        'Crescente'
        'Decrescente')
      TabOrder = 1
      OnClick = RadioGroup2Click
    end
  end
  object DSUsuario1: TDataSource
    DataSet = Usuario1
    Left = 425
    Top = 17
  end
  object Usuario1: TUsuario
    OnCalcFields = Usuario1CalcFields
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT* '
      'FROM USUARIO'
      'WHERE USU_SITUACAO<>1'
      'ORDER BY CNC_CODIGO,USU_SITUACAO DESC,USU_NOME ASC')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Selecao.Strings = (
      ' AND CNC_CODIGO=:CNC_CODIGO')
    Tabela = 'USUARIO'
    MaquinaLogado = 0
    Perfil = False
    Situacao = 0
    TodosCentros = False
    Left = 395
    Top = 17
    object Usuario1CNC_CODIGO: TIntegerField
      FieldName = 'CNC_CODIGO'
      Origin = 'DBCOMSOL.USUARIO.CNC_CODIGO'
    end
    object Usuario1USU_CODIGO: TIntegerField
      FieldName = 'USU_CODIGO'
      Origin = 'DBCOMSOL.USUARIO.USU_CODIGO'
    end
    object Usuario1USU_DT_CADASTRO: TDateTimeField
      FieldName = 'USU_DT_CADASTRO'
      Origin = 'DBCOMSOL.USUARIO.USU_DT_CADASTRO'
    end
    object Usuario1USU_DT_ALTERADO: TDateTimeField
      FieldName = 'USU_DT_ALTERADO'
      Origin = 'DBCOMSOL.USUARIO.USU_DT_ALTERADO'
    end
    object Usuario1USU_NOME: TStringField
      FieldName = 'USU_NOME'
      Origin = 'DBCOMSOL.USUARIO.USU_NOME'
    end
    object Usuario1USU_SENHA: TStringField
      FieldName = 'USU_SENHA'
      Origin = 'DBCOMSOL.USUARIO.USU_SENHA'
    end
    object Usuario1USU_PERFIL: TSmallintField
      FieldName = 'USU_PERFIL'
      Origin = 'DBCOMSOL.USUARIO.USU_PERFIL'
    end
    object Usuario1USU_MAQUINA: TIntegerField
      FieldName = 'USU_MAQUINA'
      Origin = 'DBCOMSOL.USUARIO.USU_MAQUINA'
    end
    object Usuario1USU_SITUACAO: TIntegerField
      FieldName = 'USU_SITUACAO'
      Origin = 'DBCOMSOL.USUARIO.USU_SITUACAO'
    end
    object Usuario1USU_OBSERVACAO: TStringField
      FieldName = 'USU_OBSERVACAO'
      Origin = 'DBCOMSOL.USUARIO.USU_OBSERVACAO'
      Size = 255
    end
    object Usuario1Alterado: TStringField
      FieldKind = fkCalculated
      FieldName = 'Alterado'
      Calculated = True
    end
    object Usuario1Situacao: TStringField
      FieldKind = fkCalculated
      FieldName = 'Situacao'
      Calculated = True
    end
    object Usuario1Enter_Tab: TStringField
      FieldKind = fkCalculated
      FieldName = 'Enter_Tab'
      Calculated = True
    end
    object Usuario1Perfil: TStringField
      FieldKind = fkCalculated
      FieldName = 'Perfil'
      Calculated = True
    end
    object Usuario1Maquina: TStringField
      FieldKind = fkCalculated
      FieldName = 'Maquina'
      Calculated = True
    end
  end
  object Timer1: TTimer
    Interval = 100
    OnTimer = Timer1Timer
    Left = 455
    Top = 17
  end
  object Timer2: TTimer
    Interval = 2000
    OnTimer = Timer2Timer
    Left = 486
    Top = 17
  end
end
