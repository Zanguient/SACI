...
var
  fMxSTransport: TfMxSTransport;

implementation

uses
  UDM, MxPedido, MxCotacao, MxIntencao, MxPedCompra, MxTransferencia,
  MxAbastecimento, MxSConfPed, MxSEstEntrada, MxSEstSaida;

{$R *.DFM}

procedure TfMxSTransport.BitBtn1Click(Sender: TObject);
begin
  ...
  End;
  If fMxSPedProd.tag=7 then
  Begin
    fMxSEstEntrada.Edit10.text:=IntToStr(Entidade.CodigoPRD);
    fMxSEstEntrada.Edit11.text:=Entidade.Descricao;
    fMxSEstEntrada.Edit12.text:=Entidade.Unidade;
    fMxSEstEntrada.Edit13.text:=Entidade.Referencia;
    fMxSEstEntrada.Edit14.text:=Entidade.Marca;
  End;
  If fMxSPedProd.tag=8 then
  Begin
    fMxSEstSaida.Edit10.text:=IntToStr(Entidade.CodigoPRD);
    fMxSEstSaida.Edit11.text:=Entidade.Descricao;
    fMxSEstSaida.Edit12.text:=Entidade.Unidade;
    fMxSEstSaida.Edit13.text:=Entidade.Referencia;
    fMxSEstSaida.Edit14.text:=Entidade.Marca;
  End;
...
procedure TfMxSTransport.FormClose(Sender: TObject;
...
procedure TfMxSTransport.BitBtn2Click(Sender: TObject);
begin
  fMxSPedProd.Close;
end;

procedure TfMxSTransport.FormCreate(Sender: TObject);
begin
  Entidade:=Produto1;
