...
//    Raise Exception.Create('Venda à Vista!');
...
begin
  ...
  If (DM.Usuario1.Permissoes.IndexOf(IntToStr(6180)) < 0) and
    ...
    Raise Exception.Create('Usuário não tem permissão para Emitir Nota/Cupom!')
  else if (DM.Usuario1.Permissoes.IndexOf(IntToStr(6180)) < 0) and
     (DM.Configuracao1.Empresa = empLBM) and  //Todos da LBM podem imprimir Nota de Cartão
     (Entidade.TipoPag<>13) and    //Cartão
  ...
  Begin
    ShowMessage('Nota já impressa! Se Necessário Imprima Nota de Simples Remessa!');
    JaFaturado := True;
  end
  ...
  Begin
    if (self.Entidade.TipoDeVenda = 0) or   //Normal
       ...
       (self.Entidade.TipoDeVenda = 10) then //Serviço LBM
      Tag := 1
    else if self.Entidade.TipoDeVenda = 2 then //Remessa
      Tag := 5
    else
      Raise exception.Create('Tipo de Venda não esperada!');
    with Cliente1 do
    Begin
    ...
    Begin
      ...
      Begin
        ...
        Begin
          if (DM.Configuracao1.Empresa <> empMotical) and //Motical pode Emitir Nota para Pessoa Física
             (DM.Configuracao1.Empresa in [empLBM]) and //Pois não tem Série D
        ...
        else if self.Cliente1.CGFRG <> '' then  //Pessoa Jurídica com CGF Somente Nota
        Begin
          rdgNota.Checked := True;
          rdgCupom.Enabled := False;
        end;
...
begin
  ...
  else if cmbCampo.ItemIndex = 9 then //Razão Social
  Begin
    Entidade.Close;
