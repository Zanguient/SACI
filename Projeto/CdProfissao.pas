...
uses
  ...
  Menus, JPEG, Profissao;

type
  TfCadProfissao = class(TForm)
    Panel14: TPanel;
    DBGrid5: TDBGrid;
    Panel13: TPanel;
    Panel1: TPanel;
    ...
    procedure Profissao1AfterScroll(DataSet: TDataSet);
  private
...
procedure TfCadProfissao.SB_PrimClick(Sender: TObject);
begin
...
procedure TfCadProfissao.SB_AntClick(Sender: TObject);
begin
 if Entidade.IsEmpty then
   abort;
 If Entidade.bof Then
...
procedure TfCadProfissao.SB_ProxClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
   abort;
...
procedure TfCadProfissao.SB_UltClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
   abort;
  If Entidade.eof Then
...
procedure TfCadProfissao.AtualizaControles(Habilitar:Boolean);
begin
...
procedure TfCadProfissao.SB_NovoCabClick(Sender: TObject);
begin
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
    raise Exception.Create('Você não pode inserir registro deste centro de custo.');
  If DM.Usuario1.Permissoes.IndexOf(IntToStr(8690)) < 0 then
...
procedure TfCadProfissao.SB_AltCabClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
    raise exception.Create('Não há dados a serem alterados!');
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
    raise Exception.Create('Você não pode alterar registro deste centro de custo.');
  If DM.Usuario1.Permissoes.IndexOf(IntToStr(8700)) < 0 then
...
procedure TfCadProfissao.SB_ExcCabClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
    raise exception.Create('Não há dados a serem excluídos!');
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
    raise Exception.Create('Você não pode excluir registro deste centro de custo.');
  If DM.Usuario1.Permissoes.IndexOf(IntToStr(8710)) < 0 then
  ...
  Begin
    //Verificar a existência de CLIENTES com essa profissão
    Close;
    SQL.Text := 'SELECT CLI_CODIGO '+
      ' FROM CLIENTE '+
      ' WHERE PRF_CODIGO='+IntToStr(Entidade.CodigoPRF);
    Open;
    if not IsEmpty then
      Raise exception.Create('Existem Clientes com essa profissão! ('+FieldByName('CLI_CODIGO').asString+')');
  end;
  *)
...
procedure TfCadProfissao.SB_SalvaCabClick(Sender: TObject);
begin
...
procedure TfCadProfissao.SB_CancCabClick(Sender: TObject);
begin
...
procedure TfCadProfissao.FormCreate(Sender: TObject);
begin
  Entidade:=Profissao1;
  Entidade.Open;
end;

procedure TfCadProfissao.SB_LocCabClick(Sender: TObject);
begin
...
procedure TfCadProfissao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Entidade.Active:=False;
  Action:=Cafree;
end;

procedure TfCadProfissao.SairCabClick(Sender: TObject);
begin
 Close;
end;

procedure TfCadProfissao.FormCloseQuery(Sender: TObject;
...
procedure TfCadProfissao.FormShow(Sender: TObject);
begin
...
procedure TfCadProfissao.FormKeyPress(Sender: TObject; var Key: Char);
begin
  If key=chr(27) then
  Begin
...
procedure TfCadProfissao.Profissao1AfterScroll(DataSet: TDataSet);
begin
