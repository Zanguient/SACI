object frmCarta: TfrmCarta
  Left = 185
  Top = 189
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Descri'#231#227'o da Carta'
  ClientHeight = 178
  ClientWidth = 934
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Panel5: TPanel
    Left = 0
    Top = 138
    Width = 934
    Height = 40
    Align = alBottom
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 0
    DesignSize = (
      934
      40)
    object SB_Retorno: TSpeedButton
      Left = 836
      Top = 7
      Width = 90
      Height = 25
      Hint = 'Fazer Pagamento dos T'#237'tulos em Cobran'#231'a'
      Anchors = [akTop, akRight]
      Caption = '&Confirmar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFD}
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      OnClick = SB_RetornoClick
    end
  end
  object RichEdit1: TRichEdit
    Left = 0
    Top = 0
    Width = 934
    Height = 138
    Align = alClient
    Lines.Strings = (
      
        '                           Aten'#231#227'o cliente amigo, voc'#234' que est'#225' ' +
        'com sua situa'#231#227'o irregular junto '#224' Luciano '#211'tica, venha a nossa ' +
        'loja e regularize seu d'#233'bito com as melhores condi'#231#245'es de pagame' +
        'nto  e'
      
        'totalmente sem juros. E tem mais, aproveite para participar da s' +
        'uper promo'#231#227'o Luciano '#211'tica aumenta seu 13'#176' sal'#225'rio, onde a cada' +
        ' R$ 30,00 em parcelas pagas voc'#234' cliente ganha um cupom para'
      'concorrer a R$ 10.000,00 em dinheiro no final do ano.'
      
        '                           Essa '#233' a sua oportunidade de retomar ' +
        'seu cr'#233'dito e aproveitar para participar dessa super promo'#231#227'o.'
      ''
      '  Luciano '#211'tica, 50 anos de olho em voc'#234'!!'
      ''
      '  Atenciosamente,'
      '  ______________'
      '  Setor Cobran'#231'as')
    TabOrder = 1
  end
end
