...
type
  TfMxPedidoMotCal = class(TForm)
    ...
    procedure dbgItensKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure sbConsignacaoClick(Sender: TObject);
...
var
  fMxPedidoMotCal: TfMxPedidoMotCal;
...
Uses
  ...
  UComprovanteMovimento, CdLote, MxSConsignacao, MxConsignacao;

{$R *.DFM}

procedure TfMxPedidoMotCal.FormCreate(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  try
...
procedure TfMxPedidoMotCal.edtCodigoPRDExit(Sender: TObject);
var
  Foto:string;
...
begin
  ...
  begin
    ...
    //Anderson Gonçalves - 21/02/2013
    ParamByName('PRD_CODIGO').AsString := edtCodigoPRD.Text;
    Open;
  end;
  ...
  begin
    ...
    else if Item_de_Estoque_Fiscal1.FieldByName('IES_SFISCAL').AsFloat > 0.0 then
      ckbFiscal.Checked := True
    else
      ckbFiscal.Checked := False;
  end;
...
procedure TfMxPedidoMotCal.Item_Pedido_Venda1AfterScroll(DataSet: TDataSet);
begin
  try
  ...
  begin
    ...
    if qrConsultas.Fields[0].AsInteger = 1 then
      ckbFiscal.Checked := True
    else if qrConsultas.Fields[0].AsInteger = 0 then
      ckbFiscal.Checked := False
    else
      ckbFiscal.Checked := False;
    //
...
procedure TfMxPedidoMotCal.FormCloseQuery(Sender: TObject;
...
procedure TfMxPedidoMotCal.dblFormaPagamentoClick(Sender: TObject);
var
  PDV,CNC:integer;
  aux:TDataSetNotifyEvent;
begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        SQL.Text:='SELECT * FROM AGENTE_FINANCEIRO '+
          ...
          ' AND AGF_CODIGO<>1 AND AGF_CODIGO<>24'+//Losango
          ' AND (AGF_SITUACAO=0 OR AGF_SITUACAO IS NULL)';
        Open;
      End;
    ...
    //05/11/2013
    else If dblTipoPagamento.KeyValue = 495 then //Financeira
    Begin
      With AgenteFin1 do
      Begin
        Close;
      ...
      Begin
        ...
        if SB_SalvaPed.Enabled and
           (dblCodigoAGF.Text = '') then
          dblCodigoAGF.KeyValue := 495; //Losango
      end;
    end
    /////////////////////
...
procedure TfMxPedidoMotCal.FormShow(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.ckbMelhorDataClick(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.AgenteFin1FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
var
  nparc:Integer;
begin
...
procedure TfMxPedidoMotCal.AgenteFin1BeforeOpen(DataSet: TDataSet);
begin
  AgenteFin1.OnFilterRecord:=AgenteFin1FilterRecord;
  AgenteFin1.Filtered:=True;
end;

procedure TfMxPedidoMotCal.edtCodigoCLIExit(Sender: TObject);
var
  Foto: string;
begin
...
procedure TfMxPedidoMotCal.Item_Pedido_Venda1CalcFields(DataSet: TDataSet);
var
  PrecoUnit:string;
begin
  with Produto1 do
...
procedure TfMxPedidoMotCal.Titulo_receber2BeforeOpen(DataSet: TDataSet);
begin
  try
...
procedure TfMxPedidoMotCal.imgFotoClienteDblClick(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.Titulo_receber4BeforeOpen(DataSet: TDataSet);
begin
...
procedure TfMxPedidoMotCal.edtBonificacaoExit(Sender: TObject);
begin
  if not VerificaFloatBranco(edtBonificacao.Text) then
    edtBonificacao.Text:='0';
end;

procedure TfMxPedidoMotCal.Parametro2BeforeOpen(DataSet: TDataSet);
begin
  if Cliente1.Logradouro > 0 then
    Parametro2.ParamByName('PAR_CODIGO').asInteger := Cliente1.Logradouro;
end;

procedure TfMxPedidoMotCal.dbgItensDblClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
  Foto:string;
  PrecoUnit:String;
...
procedure TfMxPedidoMotCal.CarregarItemVenda;
var
  Foto:string;
Begin
  If not Item_Pedido_Venda1.IsEmpty Then
...
procedure TfMxPedidoMotCal.imgProdutoDblClick(Sender: TObject);
begin
  Application.CreateForm(TfMxFotoAmpliada, fMxFotoAmpliada);
...
procedure TfMxPedidoMotCal.edtDiasOrcamentoExit(Sender: TObject);
begin
  if not VerificaInteiroBranco(edtDiasOrcamento.Text) then
    edtDiasOrcamento.Text := '0';
end;

procedure TfMxPedidoMotCal.Produto1CalcFields(DataSet: TDataSet);
begin
...
procedure TfMxPedidoMotCal.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key=chr(13) then
  Begin
    Key := #0;
...
procedure TfMxPedidoMotCal.AtualizaControles(Cabecalho, Habilitar: Boolean);
begin
  ...
  sbConsignacao.Enabled        := not Habilitar;
end;

procedure TfMxPedidoMotCal.SB_NovoPedClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
begin
  SairPreco := True;
...
procedure TfMxPedidoMotCal.SB_LocPedClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
begin
  //if DM.Configuracao1.Empresa = empLuciano then
  ...
  //if Pedido_de_Venda1.FieldByName('CSG_CODIGO').AsInteger = -2 then
  //begin
    //showmessage('Esse pedido foi reaberto e pertence a uma consignação.' + #13 + 'Altere os itens da consignação.');
      //Consignao1.Click;
      //sbConsignacao.Click; não precisa processar a consignação nesse ponto
  //end;
end;

procedure TfMxPedidoMotCal.SB_AltPedClick(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.SB_SalvaPedClick(Sender: TObject);
Var
  Aux:TDataSetNotifyEvent;
  CdCNC, CdPDV: integer;
begin
...
procedure TfMxPedidoMotCal.SB_CancPedClick(Sender: TObject);
var
  sCliente:String;
begin
  Entidade.AtualizaControles(False);
  ...
  begin
    Entidade.LimpaCampos;
    SB_Frete.enabled := false;
    sbDetalhe.enabled := false;
    sbConsignacao.enabled := false;
    ...
    SB_ImprimirPedido.enabled := false;
    SB_Promissoria.enabled := false;
    SB_Cupom.enabled := false;
    SB_CancelarPedido.enabled := false;
  end
  else
    Entidade.CarregaDados;
...
procedure TfMxPedidoMotCal.SB_EntradaClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
begin
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
...
procedure TfMxPedidoMotCal.SB_NovoClick(Sender: TObject);
begin
  QtdAnt := 0;
...
procedure TfMxPedidoMotCal.SB_ExcClick(Sender: TObject);
var
...
procedure TfMxPedidoMotCal.SB_SalvaClick(Sender: TObject);
Var
  mensagem, sEspecial, sPaciente, TAM, sDescricaoFPG: String;
  trans, vImprimirAouD: Boolean;
  i, CdCNC, CdPDV, CdPRD, sFiscal: integer;
...
begin
  ...
  //Não permite adicionar quantidade maior do que a quantidade do item da consignação
  ...
  begin
    ...
    Begin
      ...
      if StrToFloat(edtQuantidade.Text) > DM.QR_Consultas.FieldByName('ICS_QUANTIDADE').AsFloat then
        mensagem:= mensagem + 'Quantidade do item pedido ''' + edtQuantidade.Text + ''',' +#13+
                              'maior que a quantidade existente no item da consignação ''' + DM.QR_Consultas.FieldByName('ICS_QUANTIDADE').AsString + '''!'+#13;
    end;
  ...
  begin
    try
      ...
      if Pedido_de_Venda1.FieldByName('CSG_CODIGO').AsInteger > 0 then
      begin
        ExecutaSQL(DM.QR_Comandos,'UPDATE ITEM_DE_PEDIDO_DE_VENDA SET IPV_OBSERVACAO=''CONSIGNAÇÃO'''+
      ...
      If DM.Configuracao1.ControleTransacao then
        ...
        Begin
          RollBack;
          ShowMessage('Os dados não foram salvos! Anote os códigos e ligue para o suporte' + #13 + e.Message);
        end;
    end;
  End;

  //Alan 25-04-2006 (foi colocado la em baixo)
  ...
  Begin
    if vQtdVendida <= 0 then
      Raise exception.Create('Não é permitido diminuir a quantidade de itens!')
...
procedure TfMxPedidoMotCal.SB_CancClick(Sender: TObject);
var
  aux:TNotifyEvent;
begin
...
procedure TfMxPedidoMotCal.SB_SalvarPedidoClick(Sender: TObject);
Var
...
procedure TfMxPedidoMotCal.SB_InformacoesClick(Sender: TObject);
var
...
procedure TfMxPedidoMotCal.SB_FecharPedidoClick(Sender: TObject);
begin
  Close;
end;

procedure TfMxPedidoMotCal.SB_FinalizarPedidoClick(Sender: TObject);
Var
    ...
    try
      ...
      Begin
        ...
        Begin
          ...
          //Altera o item da consignação em 09/09/2011
          with DM.QR_Consultas do
          Begin
            Close;
            SQL.Text := 'SELECT ICS_QUANTIDADE '+
            ...
            if not DM.QR_Consultas.isEmpty then
            begin
              if DM.QR_Consultas.FieldByName('ICS_QUANTIDADE').AsFloat - Quantidade = 0 then
      ...
      //altera situação da consignação
      { Não Finaliza a consignação
      if Pedido_de_Venda1.FieldByName('CSG_CODIGO').AsInteger > 0 then
      begin
        ExecutaSQL(DM.QueryExec,'UPDATE CONSIGNACAO'+
                                ' SET CSG_SITUACAO = 2'+
                                ' WHERE CNC_CODIGO = '+Pedido_de_Venda1.FieldByName('CNC_CODIGO').AsString+
                                ' AND CSG_CODIGO = '+Pedido_de_Venda1.FieldByName('CSG_CODIGO').AsString);
      end;
      }
...
procedure TfMxPedidoMotCal.DescontonoPedido1Click(Sender: TObject);
var
  CdCNC, CdPDV: integer;
begin
  if Item_Pedido_Venda1.IsEmpty then
...
procedure TfMxPedidoMotCal.Calculadora1Click(Sender: TObject);
var
  Hand1 : HWND;
begin
...
procedure TfMxPedidoMotCal.Calendrio1Click(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.SB_ImprimirPedidoClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
  CdCNC, CdPDV : integer;
  vServico: Boolean;
...
procedure TfMxPedidoMotCal.SB_CancelarPedidoClick(Sender: TObject);
var
  Trans:Boolean;
  aux:TDataSetNotifyEvent;
  vMotivo: string;
...
procedure TfMxPedidoMotCal.SB_CupomClick(Sender: TObject);
var
  aux: TDataSetNotifyEvent;
begin
  if Entidade.Situacao <> 4 then
...
procedure TfMxPedidoMotCal.SB_PromissoriaClick(Sender: TObject);
begin
...
function TfMxPedidoMotCal.RecalcularPrecos: Boolean;
var
...
procedure TfMxPedidoMotCal.ObservaoItem1Click(Sender: TObject);
var
  CdCNC, CdIPV: integer;
  sObsItem: string;
begin
...
procedure TfMxPedidoMotCal.edtPrecoDescontoChange(Sender: TObject);
begin
  try
...
procedure TfMxPedidoMotCal.edtQuantidadeExit(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.memObservacaoKeyPress(Sender: TObject;
  var Key: Char);
begin
  Key := UpCase(Key);
end;

procedure TfMxPedidoMotCal.SB_AltClick(Sender: TObject);
begin
  if DM.Configuracao1.CodigoCNC <> DM.Usuario1.CentroDeCusto then
...
procedure TfMxPedidoMotCal.SB_FreteClick(Sender: TObject);
var
  aux:TDataSetNotifyEvent;
begin
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
...
procedure TfMxPedidoMotCal.DetalhePedidoCabClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
...
procedure TfMxPedidoMotCal.PagamentoAntecipadoCabClick(Sender: TObject);
var
...
procedure TfMxPedidoMotCal.EntregaAntecipada1Click(Sender: TObject);
var
...
procedure TfMxPedidoMotCal.ExtornoEntregaAntecipada1Click(Sender: TObject);
var
  CdCNC, CdPDV, CdIPV, CdPRD, CdLOT: integer;
  Trans: Boolean;
  vQtd, vQtdSaldo, vQtdAux: Double;
...
procedure TfMxPedidoMotCal.ExtPagamentoAntecipadoCabClick(Sender: TObject);
var
  vPagAntecipado: Double;
  Trans:Boolean;
  CdCNC, CdPDV, CdCLI, CdCLICNC, CaixaCNC: integer;
...
procedure TfMxPedidoMotCal.dbgItensTitleClick(Column: TColumn);
begin
  try
...
procedure TfMxPedidoMotCal.edtDescontoChange(Sender: TObject);
Var
  Descont:Double;
begin
...
procedure TfMxPedidoMotCal.edtDescontoExit(Sender: TObject);
begin
  if (edtDesconto.Text = '') then
    edtDesconto.Text:='0';
  if (edtDesconto.Text = '0') then
...
procedure TfMxPedidoMotCal.Vendedor1BeforeOpen(DataSet: TDataSet);
begin
  try
...
procedure TfMxPedidoMotCal.edtCodigoLOTExit(Sender: TObject);
begin
...
procedure TfMxPedidoMotCal.edtCodigoLOTKeyPress(Sender: TObject;
  var Key: Char);
begin
  If key=chr(32) then
  Begin
...
procedure TfMxPedidoMotCal.Parametro4BeforeOpen(DataSet: TDataSet);
begin
  try
    Parametro4.ParamByName('PAR_INFORMACAO').asInteger := DM.Configuracao1.CodigoCNC;
  except
  end;
end;

procedure TfMxPedidoMotCal.dbgItensCellClick(Column: TColumn);
begin
  if ( DM.Configuracao1.Empresa = empTodas ) and ( DM.Configuracao1.EstoqueIdeal ) then
    ckbFiscal.Checked := True
  else
...
procedure TfMxPedidoMotCal.dbgItensKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ( DM.Configuracao1.Empresa = empTodas ) and ( DM.Configuracao1.EstoqueIdeal ) then
    ckbFiscal.Checked := True
...
procedure TfMxPedidoMotCal.sbConsignacaoClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
    Raise exception.Create('Selecione o pedido!');
  if DM.Configuracao1.CodigoCNC<>DM.Usuario1.CentroDeCusto then
...
procedure TfMxPedidoMotCal.Consignao1Click(Sender: TObject);
begin
