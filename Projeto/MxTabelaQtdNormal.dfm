object fMxTabelaQtdNormal: TfMxTabelaQtdNormal
  Left = 270
  Top = 188
  HelpContext = 3900
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Quantidades Normais de Venda'
  ClientHeight = 429
  ClientWidth = 782
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel13: TPanel
    Left = 0
    Top = 0
    Width = 782
    Height = 86
    Align = alTop
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 0
    object Label3: TLabel
      Left = 15
      Top = 7
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
    end
    object Label4: TLabel
      Left = 81
      Top = 7
      Width = 54
      Height = 13
      Caption = 'Fornecedor'
    end
    object Label6: TLabel
      Left = 336
      Top = 44
      Width = 55
      Height = 13
      Caption = 'Quantidade'
    end
    object Label5: TLabel
      Left = 618
      Top = 44
      Width = 36
      Height = 13
      Caption = 'Usu'#225'rio'
    end
    object Label9: TLabel
      Left = 336
      Top = 7
      Width = 29
      Height = 13
      Caption = 'Grupo'
    end
    object Label10: TLabel
      Left = 147
      Top = 7
      Width = 63
      Height = 13
      Caption = 'Raz'#227'o Social'
    end
    object Label12: TLabel
      Left = 15
      Top = 44
      Width = 37
      Height = 13
      Caption = 'Produto'
    end
    object Label13: TLabel
      Left = 552
      Top = 7
      Width = 48
      Height = 13
      Caption = 'SubGrupo'
    end
    object Label8: TLabel
      Left = 512
      Top = 44
      Width = 39
      Height = 13
      Caption = 'Alterado'
    end
    object Label11: TLabel
      Left = 402
      Top = 44
      Width = 42
      Height = 13
      Caption = 'Cadastro'
    end
    object Label7: TLabel
      Left = 81
      Top = 44
      Width = 48
      Height = 13
      Caption = 'Descri'#231#227'o'
    end
    object Label14: TLabel
      Left = 402
      Top = 7
      Width = 48
      Height = 13
      Caption = 'Descri'#231#227'o'
    end
    object Label15: TLabel
      Left = 618
      Top = 7
      Width = 48
      Height = 13
      Caption = 'Descri'#231#227'o'
    end
    object edtCodigoQNV: TEdit
      Left = 15
      Top = 21
      Width = 65
      Height = 21
      Hint = 'Codigo Sequencial'
      Color = clMenu
      Enabled = False
      TabOrder = 0
    end
    object edtQuantidade: TEdit
      Left = 336
      Top = 58
      Width = 65
      Height = 21
      CharCase = ecUpperCase
      Enabled = False
      TabOrder = 9
    end
    object edtCodigoFOR: TEdit
      Left = 81
      Top = 21
      Width = 65
      Height = 21
      Enabled = False
      TabOrder = 1
      OnExit = edtCodigoFORExit
      OnKeyPress = edtCodigoFORKeyPress
    end
    object edtFornecedor: TEdit
      Left = 147
      Top = 21
      Width = 188
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 2
    end
    object edtUsuario: TEdit
      Left = 618
      Top = 58
      Width = 150
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 12
    end
    object edtCodigoPRD: TEdit
      Left = 15
      Top = 58
      Width = 65
      Height = 21
      Enabled = False
      TabOrder = 7
      Text = '0'
      OnExit = edtCodigoPRDExit
    end
    object edtCodigoGRP: TEdit
      Left = 336
      Top = 21
      Width = 65
      Height = 21
      Enabled = False
      TabOrder = 3
      Text = '0'
      OnExit = edtCodigoGRPExit
    end
    object edtGrupo: TEdit
      Left = 402
      Top = 21
      Width = 149
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 4
    end
    object edtCodigoSGP: TEdit
      Left = 552
      Top = 21
      Width = 65
      Height = 21
      Enabled = False
      TabOrder = 5
      Text = '0'
      OnExit = edtCodigoSGPExit
    end
    object edtSubGrupo: TEdit
      Left = 618
      Top = 21
      Width = 150
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 6
    end
    object edtProduto: TEdit
      Left = 81
      Top = 58
      Width = 254
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 8
    end
    object mskAlterado: TMaskEdit
      Left = 511
      Top = 58
      Width = 106
      Height = 21
      Color = clMenu
      Enabled = False
      EditMask = '!99/99/9999 99:99;1;_'
      MaxLength = 16
      TabOrder = 11
      Text = '  /  /       :  '
    end
    object mskCadastro: TMaskEdit
      Left = 402
      Top = 58
      Width = 108
      Height = 21
      Color = clMenu
      Enabled = False
      EditMask = '!99/99/9999 99:99;1;_'
      MaxLength = 16
      TabOrder = 10
      Text = '  /  /       :  '
    end
  end
  object Panel14: TPanel
    Left = 0
    Top = 86
    Width = 782
    Height = 343
    Align = alClient
    BevelInner = bvLowered
    BevelOuter = bvNone
    Caption = 'Panel14'
    TabOrder = 1
    object DBGrid5: TDBGrid
      Left = 1
      Top = 45
      Width = 780
      Height = 297
      Align = alClient
      DataSource = DSQuantidadeNormalVenda1
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'QNV_CODIGO'
          Title.Caption = 'C'#243'digo'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOR_CODIGO'
          Title.Caption = 'Fornecedor'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Fornecedor'
          Title.Caption = 'Raz'#227'o Social'
          Width = 160
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Grupo'
          Width = 125
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'SubGrupo'
          Width = 126
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Produto'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QNV_QUANTIDADE'
          Title.Caption = 'Quantidade'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QNV_DT_CADASTRO'
          Title.Caption = 'Cadastro'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QNV_DT_ALTERADO'
          Title.Caption = 'Alterado'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QNV_SITUACAO'
          Title.Caption = 'Situa'#231#227'o'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Usuario'
          Title.Caption = 'Usu'#225'rio'
          Visible = True
        end>
    end
    object Panel1: TPanel
      Left = 1
      Top = 1
      Width = 780
      Height = 44
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
      object SB_Loc: TSpeedButton
        Left = 415
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Localizar'
        Glyph.Data = {
          
          B1B1B1B1B1B1B1B1B1B1B1999999FF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = SB_LocClick
      end
      object SB_Prim: TSpeedButton
        Left = 285
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Primeiro'
        Glyph.Data = {
          
          CCC333333333333777FF33333333333333CC3333333333333773}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        Visible = False
        OnClick = SB_PrimClick
      end
      object Label1: TLabel
        Left = 447
        Top = 1
        Width = 42
        Height = 13
        Caption = '&Localizar'
        FocusControl = Edit11
      end
      object Label2: TLabel
        Left = 640
        Top = 1
        Width = 33
        Height = 13
        Caption = 'Campo'
      end
      object SB_Ant: TSpeedButton
        Left = 310
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Anterior'
        Glyph.Data = {
          
          3333333333333333333333333333333333333333333333333333}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        Visible = False
        OnClick = SB_AntClick
      end
      object SB_Prox: TSpeedButton
        Left = 335
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Pr'#243'ximo'
        Glyph.Data = {
          
          3333333333333333333333333333333333333333333333333333}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        Visible = False
        OnClick = SB_ProxClick
      end
      object SB_Ult: TSpeedButton
        Left = 360
        Top = 10
        Width = 26
        Height = 26
        Hint = #218'ltimo'
        Glyph.Data = {
          
          33333777333333333333CC333333333333337733333333333333}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        Visible = False
        OnClick = SB_UltClick
      end
      object SB_Novo: TSpeedButton
        Left = 15
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Novo'
        Glyph.Data = {
          
          B1B1B1B1B1B1B1B1B1B1B1999999FF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = SB_NovoClick
      end
      object SB_Alt: TSpeedButton
        Left = 40
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Alterar'
        Glyph.Data = {
          
          EBEBEBDBFDFDFD570EFD070707070707070707F7FDFDFDF7A4FD}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = SB_AltClick
      end
      object SB_Exc: TSpeedButton
        Left = 65
        Top = 10
        Width = 26
        Height = 26
        Hint = 'Excluir'
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = SB_ExcClick
      end
      object Edit11: TEdit
        Left = 446
        Top = 15
        Width = 194
        Height = 21
        TabOrder = 2
      end
      object ComboBox1: TComboBox
        Left = 641
        Top = 15
        Width = 126
        Height = 19
        Style = csOwnerDrawFixed
        ItemHeight = 13
        TabOrder = 3
        Items.Strings = (
          
            'C'#243'digo                                                          ' +
            '              QNV_CODIGO'
          
            'C'#243'd. Fornecedor                                                 ' +
            '        FOR_CODIGO'
          
            'C'#243'd. Grupo                                                      ' +
            '           GRP_CODIGO'
          
            'C'#243'd. SubGrupo                                                   ' +
            '        SGP_CODIGO'
          
            'C'#243'd. Produto                                                    ' +
            '           PRD_CODIGO')
      end
      object SB_Salva: TBitBtn
        Left = 103
        Top = 10
        Width = 26
        Height = 26
        Default = True
        Enabled = False
        TabOrder = 0
        OnClick = SB_SalvaClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
      object SB_Canc: TBitBtn
        Left = 128
        Top = 10
        Width = 26
        Height = 26
        Enabled = False
        TabOrder = 1
        OnClick = SB_CancClick
        Glyph.Data = {
          
          FDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFD}
        NumGlyphs = 2
      end
    end
  end
  object Grupo1: TGrupo
    DatabaseName = 'DBguerra'
    Filtered = True
    SQL.Strings = (
      'SELECT GRP_CODIGO,GRP_DESCRICAO '
      'FROM GRUPO'
      'WHERE GRP_CODIGO=:GRP_CODIGO')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'GRUPO'
    CodigoGRP = 0
    Especial = False
    Servico = False
    Situacao = 0
    Left = 286
    Top = 171
    ParamData = <
      item
        DataType = ftInteger
        Name = 'GRP_CODIGO'
        ParamType = ptUnknown
      end>
  end
  object Fornecedor1: TFornecedor
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT FOR_CODIGO,FOR_RZ_SOCIAL'
      'FROM FORNECEDOR'
      'WHERE FOR_CODIGO=:FOR_CODIGO')
    CConfiguracao = DM.Configuracao1
    CLog = DM.Log1
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'FORNECEDOR'
    Banco = 0
    CodigoCCX = 0
    CodigoFOR = 0
    Logradouro = 0
    MaxItensNota = 0
    Situacao = 0
    Left = 330
    Top = 171
    ParamData = <
      item
        DataType = ftInteger
        Name = 'FOR_CODIGO'
        ParamType = ptUnknown
      end>
  end
  object DSQuantidadeNormalVenda1: TDataSource
    DataSet = QuantidadeNormalVenda1
    Left = 112
    Top = 171
  end
  object MainMenu1: TMainMenu
    Left = 69
    Top = 171
    object Sair1: TMenuItem
      Caption = '&Fechar'
      OnClick = Sair1Click
    end
    object Clientes1: TMenuItem
      Caption = 'Cabe'#231'alho'
      object Incluir1: TMenuItem
        Caption = 'Incluir'
        ShortCut = 116
        OnClick = SB_NovoClick
      end
      object Alterar1: TMenuItem
        Caption = 'Alterar'
        ShortCut = 117
        OnClick = SB_AltClick
      end
      object Excluir1: TMenuItem
        Caption = 'Excluir'
        ShortCut = 119
        OnClick = SB_ExcClick
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object Salvar1: TMenuItem
        Caption = 'Salvar'
        Enabled = False
        ShortCut = 120
        OnClick = SB_SalvaClick
      end
      object Cancelar1: TMenuItem
        Caption = 'Cancelar'
        Enabled = False
        ShortCut = 121
        OnClick = SB_CancClick
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object Localizar1: TMenuItem
        Caption = 'Localizar'
        ShortCut = 114
        OnClick = SB_LocClick
      end
    end
  end
  object SubGrupo1: TSubGrupo
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT * FROM SUBGRUPO'
      'WHERE GRP_CODIGO=:GRP_CODIGO'
      'AND SGP_CODIGO=:SGP_CODIGO')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'SUBGRUPO'
    CodigoGRP = 0
    CodigoSGP = 0
    Fabricar = False
    PrecoDolar = False
    ServicoObrigatorio = False
    Situacao = 0
    Left = 199
    Top = 171
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'GRP_CODIGO'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'SGP_CODIGO'
        ParamType = ptUnknown
      end>
  end
  object QuantidadeNormalVenda1: TQuantidadeNormalVenda
    AfterScroll = QuantidadeNormalVenda1AfterScroll
    OnCalcFields = QuantidadeNormalVenda1CalcFields
    DatabaseName = 'DBguerra'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM QUANTIDADE_NORMAL_VENDA'
      'ORDER BY FOR_CODIGO, GRP_CODIGO,SGP_CODIGO, PRD_CODIGO')
    CConfiguracao = DM.Configuracao1
    CLog = DM.Log1
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    ECampoLocaliz = ComboBox1
    EDbgrid = DBGrid5
    ENomeLocaliz = Edit11
    ESCodigo = edtCodigoQNV
    Mafalda = 0
    Maquina = 1
    Tabela = 'QUANTIDADE_NORMAL_VENDA'
    CodigoFOR = 0
    CodigoGRP = 0
    CodigoPRD = 0
    CodigoQNV = 0
    CodigoSGP = 0
    ECodigoFOR = edtCodigoFOR
    ECodigoGRP = edtCodigoGRP
    ECodigoPRD = edtCodigoPRD
    ECodigoQNV = edtCodigoQNV
    ECodigoSGP = edtCodigoSGP
    EDataAlterado = mskAlterado
    EDataCadastro = mskCadastro
    EQuantidade = edtQuantidade
    Situacao = 0
    Left = 156
    Top = 171
    object QuantidadeNormalVenda1CNC_CODIGO: TIntegerField
      FieldName = 'CNC_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.CNC_CODIGO'
    end
    object QuantidadeNormalVenda1QNV_CODIGO: TIntegerField
      FieldName = 'QNV_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_CODIGO'
    end
    object QuantidadeNormalVenda1USU_CODIGO: TIntegerField
      FieldName = 'USU_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.USU_CODIGO'
    end
    object QuantidadeNormalVenda1FOR_CODIGO: TIntegerField
      FieldName = 'FOR_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.FOR_CODIGO'
    end
    object QuantidadeNormalVenda1GRP_CODIGO: TIntegerField
      FieldName = 'GRP_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.GRP_CODIGO'
    end
    object QuantidadeNormalVenda1SGP_CODIGO: TIntegerField
      FieldName = 'SGP_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.SGP_CODIGO'
    end
    object QuantidadeNormalVenda1PRD_CODIGO: TIntegerField
      FieldName = 'PRD_CODIGO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.PRD_CODIGO'
    end
    object QuantidadeNormalVenda1QNV_DT_CADASTRO: TDateTimeField
      FieldName = 'QNV_DT_CADASTRO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_DT_CADASTRO'
    end
    object QuantidadeNormalVenda1QNV_DT_ALTERADO: TDateTimeField
      FieldName = 'QNV_DT_ALTERADO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_DT_ALTERADO'
    end
    object QuantidadeNormalVenda1QNV_QUANTIDADE: TFloatField
      FieldName = 'QNV_QUANTIDADE'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_QUANTIDADE'
    end
    object QuantidadeNormalVenda1QNV_SITUACAO: TIntegerField
      FieldName = 'QNV_SITUACAO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_SITUACAO'
    end
    object QuantidadeNormalVenda1QNV_OBSERVACAO: TStringField
      FieldName = 'QNV_OBSERVACAO'
      Origin = 'DBGUERRA.QUANTIDADE_NORMAL_VENDA.QNV_OBSERVACAO'
      FixedChar = True
      Size = 255
    end
    object QuantidadeNormalVenda1Produto: TStringField
      FieldKind = fkCalculated
      FieldName = 'Produto'
      Size = 50
      Calculated = True
    end
    object QuantidadeNormalVenda1Fornecedor: TStringField
      FieldKind = fkCalculated
      FieldName = 'Fornecedor'
      Size = 30
      Calculated = True
    end
    object QuantidadeNormalVenda1Grupo: TStringField
      FieldKind = fkCalculated
      FieldName = 'Grupo'
      Size = 30
      Calculated = True
    end
    object QuantidadeNormalVenda1SubGrupo: TStringField
      FieldKind = fkCalculated
      FieldName = 'SubGrupo'
      Size = 30
      Calculated = True
    end
    object QuantidadeNormalVenda1Usuario: TStringField
      FieldKind = fkCalculated
      FieldName = 'Usuario'
      Calculated = True
    end
  end
  object QuantidadeNormalVenda2: TQuantidadeNormalVenda
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT * FROM QUANTIDADE_NORMAL_VENDA'
      'ORDER BY FOR_CODIGO, GRP_CODIGO,SGP_CODIGO, PRD_CODIGO')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'QUANTIDADE_NORMAL_VENDA'
    CodigoFOR = 0
    CodigoGRP = 0
    CodigoPRD = 0
    CodigoQNV = 0
    CodigoSGP = 0
    Situacao = 0
    Left = 243
    Top = 171
  end
  object Produto1: TProduto
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT PRD_CODIGO,GRP_CODIGO,SGP_CODIGO,PRD_DESCRICAO'
      'FROM PRODUTO'
      'WHERE PRD_CODIGO=:PRD_CODIGO')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'PRODUTO'
    Catalogo = False
    Classificacao = 0
    CodigoCLF = 0
    CodigoCTP = 0
    CodigoFOR = 0
    CodigoGRP = 0
    CodigoORF = 0
    CodigoPRD = 0
    CodigoSGP = 0
    EstoqueDetalhado = False
    ForaDeLinha = False
    ForaPrevisao = False
    ForaTabela = False
    Insumo = False
    Nivel = 0
    Previsao = 0
    Servico = False
    Situacao = 0
    TempoMedio = 0
    Left = 374
    Top = 171
    ParamData = <
      item
        DataType = ftInteger
        Name = 'PRD_CODIGO'
        ParamType = ptUnknown
      end>
  end
end
