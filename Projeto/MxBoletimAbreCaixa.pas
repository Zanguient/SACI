...
type
  TfMxBoletimAbreCaixa = class(TForm)
    Panel13: TPanel;
    Label1: TLabel;
    edtCodigoBDL: TEdit;
    Label8: TLabel;
...
var
  fMxBoletimAbreCaixa: TfMxBoletimAbreCaixa;

implementation
Uses
  UDM, Funcoes, UBoletim;
{$R *.DFM}



procedure TfMxBoletimAbreCaixa.SB_PrimClick(Sender: TObject);
begin
...
procedure TfMxBoletimAbreCaixa.SB_AntClick(Sender: TObject);
begin
 if Entidade.IsEmpty then
   abort;
 If Entidade.bof Then
...
procedure TfMxBoletimAbreCaixa.SB_ProxClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
   abort;
...
procedure TfMxBoletimAbreCaixa.SB_UltClick(Sender: TObject);
begin
  if Entidade.IsEmpty then
   abort;
  If Entidade.eof Then
...
procedure TfMxBoletimAbreCaixa.SB_NovoPedClick(Sender: TObject);
begin
...
procedure TfMxBoletimAbreCaixa.SB_SalvaPedClick(Sender: TObject);
Var
  Mensagem:String;
  ...
  begin
    try
      ...
      Begin
        ...
        if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,DM.Configuracao1.ContaAbreCaixa,
           DM.Configuracao1.ContaAbreCaixa,DM.Configuracao1.CodigoCAX,
           5 (*TipoPag-Dinheiro*),68(*TipoDoc*),0,0 (*CdCHQCNC*),0 (*CdCHQ*),
           'ABERTURA CAIXA '+IntToStr(DM.Configuracao1.CodigoCAX)+'(DI)'+
           formatDateTime('hh:mm',DM.Configuracao1.DataHora),'0',
           ''(*Número*),''(*Obs*),''(*Seq*),'C',vTotalDinheiro(*Valor*),0(*Desconto*),0(*Juros*),
           0(*CredCli*),vTotalDinheiro(*ValorDoc*),
           DM.Configuracao1.DataHora,DM.Configuracao1.Data(*Competência*),
           0(*DtCheque*),0(*CdTit*),0(*CdCNCTit*),'B') then
        Begin
      ...
      Begin
        ...
        if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,DM.Configuracao1.ContaAbreCaixa,
           ...
           'ABERTURA CAIXA '+IntToStr(DM.Configuracao1.CodigoCAX)+'(CH)'+
           formatDateTime('hh:mm',DM.Configuracao1.DataHora),'0',
           ''(*Número*),''(*Obs*),''(*Seq*),'C',vTotalCheque,0(*Desconto*),0(*Juros*),
      ...
      Begin
        if DM.Configuracao1.Empresa <> empMotical then //Não alterar o Saldo do Caixa
        Begin
          //Incluir o movimento de caixa
          if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,DM.Configuracao1.ContaAbreCaixa,
             DM.Configuracao1.ContaAbreCaixa,DM.Configuracao1.CodigoCAX,
             6,68,0,0 (*CdCHQCNC*),0 (*CdCHQ*),
             'ABERTURA CAIXA '+IntToStr(DM.Configuracao1.CodigoCAX)+
             '(Pre:'+format('%.2f',[vTotalPreDatado])+')'+
             formatDateTime('hh:mm',DM.Configuracao1.DataHora),'0',
             ''(*Número*),''(*Obs*),''(*Seq*),'C',0(*Valor*),0(*Desconto*),0(*Juros*),
        ...
        Begin
          ...
          if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,DM.Configuracao1.ContaAbreCaixa,
             ...
             '(Pre-Datado)'+
             formatDateTime('hh:mm',DM.Configuracao1.DataHora),'0',
             ''(*Número*),''(*Obs*),''(*Seq*),'C',vTotalPreDatado(*Valor*),0(*Desconto*),0(*Juros*),
             0(*CredCli*),vTotalPreDatado(*ValorDoc*),
             DM.Configuracao1.DataHora,DM.Configuracao1.Data(*Competência*),
             0(*DtCheque*),0(*CdTit*),0(*CdCNCTit*),'B') then
          Begin
      ...
      Begin
        ...
        Begin
          if not DM.Movimento_caixa1.Inserir(DM.Configuracao1.CodigoCNC,0,DM.Configuracao1.ContaAbreCaixa,
             DM.Configuracao1.ContaAbreCaixa,DM.Configuracao1.CodigoCAX,
             13,68,0,0 (*CdCHQCNC*),0 (*CdCHQ*),
             'ABERTURA CAIXA '+IntToStr(DM.Configuracao1.CodigoCAX)+'(CA)'+
             formatDateTime('hh:mm',DM.Configuracao1.DataHora),'0',
             ''(*Número*),''(*Obs*),''(*Seq*),'C',vTotalCartao,0(*Desconto*),0(*Juros*),
             0(*CredCli*),vTotalCartao(*ValorDoc*),
             DM.Configuracao1.DataHora,DM.Configuracao1.Data(*Competência*),
             0(*DtCheque*),0(*CdTit*),0(*CdCNCTit*),'B') then
          Begin
...
procedure TfMxBoletimAbreCaixa.SB_CancPedClick(Sender: TObject);
begin
  Entidade.CancelaEdicao;
  AtualizaControles(1, False);
end;

procedure TfMxBoletimAbreCaixa.FormCreate(Sender: TObject);
begin
  Entidade:=Boletim_Diario1;
end;

procedure TfMxBoletimAbreCaixa.FormClose(Sender: TObject;
...
procedure TfMxBoletimAbreCaixa.CentroCusto1AfterScroll(DataSet: TDataSet);
begin
  Entidade.CarregaDados;
end;

procedure TfMxBoletimAbreCaixa.Sair1Click(Sender: TObject);
begin
  Close;
end;

procedure TfMxBoletimAbreCaixa.FormCloseQuery(Sender: TObject;
...
procedure TfMxBoletimAbreCaixa.Boletim_Diario1AfterScroll(DataSet: TDataSet);
begin
  Entidade.CarregaDados;
end;

procedure TfMxBoletimAbreCaixa.SB_ImpClick(Sender: TObject);
begin
  ...
  Begin
    ...
    Begin
      Tag := 0;
      zrlTituloBoletim.Caption := 'SIAC - Boletim Diario ABERTURA';
      Boletim_Diario1.ParamByName('BLDCOD').AsInteger     := StrToInt(edtCodigoBDL.Text);
    ...
    ShowMessage('Efetue a Abertura de Caixa!');
end;

procedure TfMxBoletimAbreCaixa.FormShow(Sender: TObject);
begin
...
procedure TfMxBoletimAbreCaixa.Boletim_Diario1BeforeOpen(DataSet: TDataSet);
begin
...
procedure TfMxBoletimAbreCaixa.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  If key=chr(27) then
  Begin
...
procedure TfMxBoletimAbreCaixa.AtualizaControles(Cabecalho: integer; Habilitar: Boolean);
begin
...
procedure TfMxBoletimAbreCaixa.Caixa1BeforeOpen(DataSet: TDataSet);
begin
  try
