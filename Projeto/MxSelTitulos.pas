...
begin
  ...
  Begin
    ...
    else if rdgEmissaoNota.Checked then //Data Emissão Nota
    Begin
  ...
  if ckbCPFCNPJ.Checked then
    if not VerificaInteiroBranco(edtCPFCNPJ.Text) then
      raise Exception.Create('Digite o CPF/CNPJ sem máscara!')
    else
    Begin
    ...
    Begin
      case cmbTipoConta.ItemIndex of
        ...
        1: Begin
          ...
          Raise exception.Create('Condição de Tipo de Conta inválido!');
      end;
    end;

  if ckbCodigoCCXRecebimento.Checked then
    if dblCodigoCCXRecebimento.Text = '' then
      raise Exception.Create('Conta Recebimento inválida!')
    else
    Begin
  ...
  if ckbCodigoAGF.Checked then
    if not (dblCodigoAGF.KeyValue > 0) then
      ShowMessage('Agente Financeiro inválido!')
    else
    Begin
      case cmbCondicaoCodigoAGF.ItemIndex of
        ...
        1: Begin
          ...
          Raise exception.Create('Condição de Agente Financeiro inválido!');
      end;
    end;
  ...
  Begin
    if not VerificaDataBranco(mskDataEnvio.Text) then
      Raise exception.Create('Data de envio ao banco inválida!');
  ...
  Begin
    if not VerificaDataBranco(mskDataRecebimento.Text) then
      Raise exception.Create('Data de retorno ao banco inválida!');
  ...
  Begin
    ...
    Begin
      ...
      Raise Exception.Create('Vencimento do Título inválido!');
    end
    else
  ...
  Begin
    ...
    Begin
      ...
      //Selecionar Título pagos com cheque Pré-Datado
      ...
      Begin
        ...
        Begin
          Titulo_receber1.SQL.Text := Titulo_receber1.SQL.Text + ' UNION '+
            ...
            ' 0 AS ORDEM '+                             //Para colocar os cheques antes dos títulos
...
//showmessage(Titulo_receber1.SQL.Text);
  ...
  else if tag = 1 then //Títulos Recebidos SQL-Server
  Begin
...
//        zrbRodapeGrupoGrupoConta.Height := 0;
      ...
      if rdgVencimento.Checked then
        ...
        Raise exception.Create('Filtro não permitido para esse relatório!');
  ...
  else if tag = 2 then  //Previsão de Receitas
  Begin
  ...
  else if tag = 3 then //Gráfico Títulos Por Vendedor
  Begin
  ...
  else if tag = 6 then //Gráfico Títulos Por Conta de Caixa ou por AGF
  Begin
  ...
  Begin
    ...
    Begin
      ...
      zrlDataINI.Caption := mskDataINI.Text; //Campo invisível
      zrlDataFIM.Caption := mskDataFIM.Text; //Campo invisível
      ...
      Entidade.SQL.Text:='SELECT '+
        ...
        ' FROM CHEQUE T1, CLIENTE T5  '+
        ' WHERE T1.CHQ_ER=''R'' '+
        ' AND (T1.CHQ_SITUACAO IN (3,4) '+ //Somente Cheques Devolvidos, Não utilizados e Pre-Datados
    ...
    Begin
      ...
      Begin
        ...
        if rdgPagamento.Checked then
          ...
          Raise exception.Create('Filtro inválido para esse relatório!');
      ...
      Titulo_receber1.SQL.Text:='SELECT DISTINCT T1.*, T2.CLI_RZ_SOCIAL, T5.CCX_GRUPO,'+
        ...
        CdFUN+CNC+CLI+sClassificacao+sTipo+CHQ+PDV+TipoPG+AGF+TRC+CdFPE+NUM+DataINI+DataFIM+CCX+CCXRec+sGrupo+cOficial+sNaoPagos+sVencimento+
        sValorTitulo+sPagCredito+sCPFCNPJ+sObs+sLocal+DtEnvBoleto+DtRecBoleto+sVerificado+
        ' AND T1.TRC_PAGAMENTO>=(T1.TRC_VENCIMENTO+'+IntToStr(iDiasAtrazo)+') '+ //Títulos Atrasados      //Títulos com mais de 60 dias de atraso
...
begin
  ...
  Begin
    if (tag <> 2) and (tag <> 3) then //Previsão de Receita
    Begin
...
begin
  ...
  if ckbInadipencia.checked then
    ...
    ckbCodigoFUN.Caption := 'Responsável';
end;
...
begin
  ...
  if tag = 2 then //Previsão de Receita
  Begin
  ...
  Begin
    ...
    cmbOrdem.ItemIndex:=0;
  if tag = 6 then //Gráfico Títulos Por Conta de Caixa ou por AGF
  Begin
  ...
  Begin
    ...
    cmbTipoPag.ItemIndex    := 1;  //<>
    dblTipoPag.KeyValue     := 13; //Cartão
    ckbSintetico.Checked    := True;
    ckbSuperSintetico.Checked := False;
  end;

  if tag <> 1 then //Diferente de Títulos Recebidos
  begin
...
begin
  stiReport.Create.ReportContaReceberSintetico(false,
                                               ...
                                               filtrosSQL,
                                               EmpresaDesenvolvedora,
                                               'Relatório Sintético de títulos a receber do sistema',
                                               DM.CentroCusto1.RazaoSocial,
                                               '',//DM.Configuracao1.LojaNome
...
begin
  stiReport.Create.ReportContaReceberSuperSintetico(false,
                                                    ...
                                                    filtrosSQL,
                                                    EmpresaDesenvolvedora,
                                                    'Relatório Super Sintético de títulos a receber do sistema',
                                                    DM.CentroCusto1.RazaoSocial,
                                                    '',//DM.Configuracao1.LojaNome
                                                    'Relatório de títulos a receber',
                                                    DM.Usuario1.NomeUSU,
