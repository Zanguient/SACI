object fMxSelBonusVendedor: TfMxSelBonusVendedor
  Left = 424
  Top = 237
  BorderIcons = [biSystemMenu]
  BorderStyle = bsDialog
  Caption = 'Selecionar B'#244'nus/Vendedor'
  ClientHeight = 287
  ClientWidth = 294
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel3: TPanel
    Left = 0
    Top = 247
    Width = 294
    Height = 40
    Align = alBottom
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 1
    DesignSize = (
      294
      40)
    object SB_Confirmar: TBitBtn
      Left = 101
      Top = 7
      Width = 90
      Height = 25
      Anchors = [akTop, akRight]
      Caption = 'Ok'
      TabOrder = 0
      OnClick = SB_ConfirmarClick
      Glyph.Data = {
        
        FDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFD}
      NumGlyphs = 2
    end
    object SB_Cancelar: TBitBtn
      Left = 190
      Top = 7
      Width = 90
      Height = 25
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = 'Cancelar'
      TabOrder = 1
      OnClick = SB_CancelarClick
      Glyph.Data = {
        
        FDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFDFD}
      NumGlyphs = 2
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 294
    Height = 247
    Align = alClient
    BevelInner = bvLowered
    BevelOuter = bvNone
    TabOrder = 0
    DesignSize = (
      294
      247)
    object Label1: TLabel
      Left = 33
      Top = 16
      Width = 56
      Height = 13
      Caption = 'Data Inicial:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 33
      Top = 42
      Width = 51
      Height = 13
      Caption = 'Data Final:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Bevel1: TBevel
      Left = 0
      Top = 65
      Width = 293
      Height = 2
      Anchors = [akLeft, akTop, akRight]
      Shape = bsFrame
    end
    object ckbCodigoCNC: TCheckBox
      Left = 15
      Top = 147
      Width = 116
      Height = 17
      Cursor = crHandPoint
      Caption = 'Centro de Custo:'
      TabOrder = 8
      OnClick = ckbCodigoCNCClick
    end
    object dblCodigoCNC: TDBLookupComboBox
      Left = 134
      Top = 145
      Width = 145
      Height = 21
      Color = clMenu
      Enabled = False
      KeyField = 'CNC_CODIGO'
      ListField = 'CNC_RZ_SOCIAL'
      ListSource = DSCentroCusto1
      TabOrder = 9
    end
    object ckbCodigoPRD: TCheckBox
      Left = 15
      Top = 103
      Width = 110
      Height = 17
      Cursor = crHandPoint
      Caption = 'Produto:'
      TabOrder = 4
      OnClick = ckbCodigoPRDClick
    end
    object edtCodigoPRD: TEdit
      Left = 134
      Top = 101
      Width = 66
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 5
      OnKeyPress = edtCodigoPRDKeyPress
    end
    object ckbSomenteBonus1: TCheckBox
      Left = 15
      Top = 170
      Width = 138
      Height = 17
      Cursor = crHandPoint
      Caption = 'Somente B'#244'nus1(%)'
      TabOrder = 10
      OnClick = ckbSomenteBonus1Click
    end
    object ckbSomenteBonus2: TCheckBox
      Left = 15
      Top = 192
      Width = 147
      Height = 17
      Cursor = crHandPoint
      Caption = 'Somente B'#244'nus2 (R$)'
      TabOrder = 11
      OnClick = ckbSomenteBonus2Click
    end
    object mskDataINI: TMaskEdit
      Left = 134
      Top = 11
      Width = 79
      Height = 21
      Hint = 'Data inicial para o Cadastro'
      EditMask = '!99/99/9999;1;_'
      MaxLength = 10
      TabOrder = 0
      Text = '  /  /    '
    end
    object mskDataFIM: TMaskEdit
      Left = 134
      Top = 35
      Width = 79
      Height = 21
      Hint = 'Data final para o Cadastro'
      EditMask = '!99/99/9999;1;_'
      MaxLength = 10
      TabOrder = 1
      Text = '  /  /    '
    end
    object dblVendedor: TDBLookupComboBox
      Left = 134
      Top = 123
      Width = 145
      Height = 21
      Color = clMenu
      Enabled = False
      KeyField = 'FUN_CODIGO'
      ListField = 'FUN_APELIDO'
      ListSource = DSVendedor1
      TabOrder = 7
    end
    object ckbVendedor: TCheckBox
      Left = 15
      Top = 125
      Width = 110
      Height = 17
      Cursor = crHandPoint
      Caption = 'Vendedor(a):'
      Enabled = False
      TabOrder = 6
      OnClick = ckbVendedorClick
    end
    object ckbCodigoPedido: TCheckBox
      Left = 15
      Top = 81
      Width = 106
      Height = 17
      Cursor = crHandPoint
      Caption = 'Pedido:'
      TabOrder = 2
      OnClick = ckbCodigoPedidoClick
    end
    object edtCodigoPedido: TEdit
      Left = 134
      Top = 79
      Width = 65
      Height = 21
      Color = clMenu
      Enabled = False
      TabOrder = 3
    end
    object ckbSintetico: TCheckBox
      Left = 15
      Top = 213
      Width = 81
      Height = 17
      Cursor = crHandPoint
      Caption = 'Sint'#233'tico'
      TabOrder = 12
    end
  end
  object CentroCusto1: TCentroCusto
    DatabaseName = 'DBguerra'
    SQL.Strings = (
      'SELECT * FROM CENTRO_DE_CUSTO'
      'ORDER BY CNC_RZ_SOCIAL')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    Tabela = 'CENTRO_DE_CUSTO'
    Logradouro = 0
    RecebeDados = False
    Left = 215
    Top = 210
  end
  object DSCentroCusto1: TDataSource
    DataSet = CentroCusto1
    Left = 247
    Top = 181
  end
  object Vendedor1: TVendedor
    BeforeOpen = Vendedor1BeforeOpen
    DatabaseName = 'DBguerra'
    Filtered = True
    SQL.Strings = (
      'SELECT FUN_CODIGO,FUN_NOME,FUN_APELIDO'
      'FROM FUNCIONARIO'
      'WHERE (FUN_FUNCAO=45 OR FUN_FUNCAO=58) AND'
      'CNC_CODIGO=:CNC_CODIGO AND FUN_SITUACAO<>1'
      'ORDER BY FUN_APELIDO'
      ' ')
    CNCOrigem = 0
    CentroDeCusto = 1
    CodigoCNC = 0
    CodigoUSU = 0
    Mafalda = 0
    Maquina = 1
    CFuncao = 0
    CLogradouro = 0
    CodigoFUN = 0
    CodigoSEC = 0
    Funcao = 0
    Logradouro = 0
    Rota = 0
    Left = 247
    Top = 210
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CNC_CODIGO'
        ParamType = ptUnknown
      end>
  end
  object DSVendedor1: TDataSource
    DataSet = Vendedor1
    Left = 215
    Top = 181
  end
end
