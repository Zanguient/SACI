...
var
  rpt_MovEstoqueSN: Trpt_MovEstoqueSN;
...
procedure Trpt_MovEstoqueSN.FormClose(Sender: TObject;
...
procedure Trpt_MovEstoqueSN.FormCreate(Sender: TObject);
begin
  Produto1.Open;
end;

procedure Trpt_MovEstoqueSN.zrbDetalheBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  zrlSaldoAnt.Caption   := format('%.2f',[Produto1.FieldByName('MSN_SALDO_ANT').asFloat]);
  zrlQuantidade.Caption := format('%.2f',[Produto1.FieldByName('MSN_QUANTIDADE').asFloat]);

  Parametro1.Close;
  Parametro1.ParamByName('PAR_CODIGO').asInteger:=Produto1.FieldByName('MSN_TIPO_MOV').asInteger;
  Parametro1.Open;
  if not Parametro1.IsEmpty then
    zrlTipoMovimento.Caption:=Parametro1.Descricao
  else
    zrlTipoMovimento.Caption:=Produto1.FieldByName('MSN_TIPO_MOV').asString;
  if Produto1.FieldByName('MSN_ES').asString = 'E' then
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MSN_SALDO_ANT').asFloat+Produto1.FieldByName('MSN_QUANTIDADE').asFloat)])
  else
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MSN_SALDO_ANT').asFloat-Produto1.FieldByName('MSN_QUANTIDADE').asFloat)]);
...
procedure Trpt_MovEstoqueSN.ZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  with Produto1 do
...
procedure Trpt_MovEstoqueSN.ZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  ZRLabel3.Caption:=DM.Configuracao1.LojaNome;
end;

procedure Trpt_MovEstoqueSN.SZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_MovEstoqueSN.ZRBand3BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
