...
type
  Trpt_MovEstoqueComercial = class(TForm)
    report: TSZReport;
    ZRBand1: TSZRBand;
    zrbDetalhe: TSZRBand;
    ZRBand3: TSZRBand;
...
var
  rpt_MovEstoqueComercial: Trpt_MovEstoqueComercial;
...
procedure Trpt_MovEstoqueComercial.FormClose(Sender: TObject;
...
procedure Trpt_MovEstoqueComercial.FormCreate(Sender: TObject);
begin
  Produto1.Open;
end;

procedure Trpt_MovEstoqueComercial.zrbDetalheBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  zrlSaldoAnt.Caption   := format('%.2f',[Produto1.FieldByName('MEC_SALDO_ANT').asFloat]);
  zrlQuantidade.Caption := format('%.2f',[Produto1.FieldByName('MEC_QUANTIDADE').asFloat]);
  ...
  if Produto1.FieldByName('MEC_ES').asString = 'E' then
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MEC_SALDO_ANT').asFloat+Produto1.FieldByName('MEC_QUANTIDADE').asFloat)])
  else
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MEC_SALDO_ANT').asFloat-Produto1.FieldByName('MEC_QUANTIDADE').asFloat)]);
...
procedure Trpt_MovEstoqueComercial.ZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  with Produto1 do
...
procedure Trpt_MovEstoqueComercial.ZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  ZRLabel3.Caption:=DM.Configuracao1.LojaNome;
end;

procedure Trpt_MovEstoqueComercial.SZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_MovEstoqueComercial.ZRBand3BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
