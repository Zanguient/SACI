...
procedure Trpt_estoque.FormClose(Sender: TObject;
...
procedure Trpt_estoque.FormCreate(Sender: TObject);
begin
  Estoque:='';
  sFiltroCNC := '';
  sFiltroEstoque := '';
...
procedure Trpt_estoque.zrbRodapeGrupoBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  DoPrint:=True;
  ...
  Begin
    ...
    if Op_RelatProd.ckbSomenteSubGrupo.Checked = true then
    begin
      DM.QuerySelect.Close;
      DM.QuerySelect.SQL.Text := 'SELECT * FROM SUBGRUPO WHERE SGP_CODIGO = '''+inttostr(produto1.CodigoSGP)+'''';
      DM.QuerySelect.Open;
      zrlCentroQuantidade.Caption := PreencheZero(6, Produto1.CodigoPRD) + ' ' +
                                     PreencheEspaco(zrlProduto.Width - 15,DM.QuerySelect.fieldbyname('SGP_DESCRICAO').AsString) +
                                     ' Saldo:'+PreencheEspacoEsq(8,format('%.2f',[SaldoEstoque])) + ' ' +
                                     PreencheEspaco(ZRDBText5.Width, Produto1.Unidade)+' '+
...
procedure Trpt_estoque.zrlDetalheBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
var
  prc:Double;
  Desc:string;
...
procedure Trpt_estoque.zrbRodapeGrupoAfterPrint(Sender: TObject;
  Printed: Boolean);
begin
  Estoque:='';
end;
...
procedure Trpt_estoque.ZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_estoque.zrbGrupoBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
var
  vQtd: Double;
begin
  ...
  begin
    ...
    DM.QuerySelect.SQL.Text := 'SELECT * FROM SUBGRUPO WHERE SGP_CODIGO = '''+inttostr(produto1.CodigoSGP)+'''';
    DM.QuerySelect.Open;
    zrlProduto.Caption := DM.QuerySelect.fieldbyname('SGP_DESCRICAO').AsString;
  end
  else
  begin
    with Produto1 do
...
procedure Trpt_estoque.SZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_estoque.Item_de_Estoque1BeforeOpen(DataSet: TDataSet);
begin
  if DM.Configuracao1.Empresa = empLBM then
    Item_de_Estoque1.ParamByName('CNC_CODIGO').asInteger:=2;
end;

procedure Trpt_estoque.ZRBand3BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
