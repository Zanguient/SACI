...
type
  Trpt_MovEstoqueAmostra = class(TForm)
    report: TSZReport;
    ZRBand1: TSZRBand;
    zrbDetalhe: TSZRBand;
    ZRBand3: TSZRBand;
...
var
  rpt_MovEstoqueAmostra: Trpt_MovEstoqueAmostra;
...
procedure Trpt_MovEstoqueAmostra.FormClose(Sender: TObject;
...
procedure Trpt_MovEstoqueAmostra.FormCreate(Sender: TObject);
begin
  Produto1.Active:=true;
end;

procedure Trpt_MovEstoqueAmostra.zrbDetalheBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  zrlSaldoAnt.Caption   := format('%.2f',[Produto1.FieldByName('MEC_AMOSTRA_ANTERIOR').asFloat]);
  zrlQuantidade.Caption := format('%.2f',[Produto1.FieldByName('MEC_AMOSTRA').asFloat]);
  ...
  if Produto1.FieldByName('MEC_ES').asString = 'E' then
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MEC_AMOSTRA_ANTERIOR').asFloat+Produto1.FieldByName('MEC_AMOSTRA').asFloat)])
  else
    SZRLabel20.Caption:=format('%.2f',[(Produto1.FieldByName('MEC_AMOSTRA_ANTERIOR').asFloat-Produto1.FieldByName('MEC_AMOSTRA').asFloat)]);
...
procedure Trpt_MovEstoqueAmostra.ZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  with Produto1 do
...
procedure Trpt_MovEstoqueAmostra.ZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  ZRLabel3.Caption:=DM.Configuracao1.LojaNome;
end;

procedure Trpt_MovEstoqueAmostra.SZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_MovEstoqueAmostra.ZRBand3BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
