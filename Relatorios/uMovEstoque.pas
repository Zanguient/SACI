...
var
  rpt_MovEstoque: Trpt_MovEstoque;
...
procedure Trpt_MovEstoque.FormClose(Sender: TObject;
...
procedure Trpt_MovEstoque.FormCreate(Sender: TObject);
begin
  Produto1.Active:=true;
  iTotalQtd := 0;  
end;

procedure Trpt_MovEstoque.zrbDetalheBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  zrlSaldoAnt.Caption   := format('%.2f',[Produto1.FieldByName('MES_SALDO_ANT').asFloat]);
  zrlQuantidade.Caption := format('%.2f',[Produto1.FieldByName('MES_QUANTIDADE').asFloat]);
  ...
  Begin
    iTotalQtd := iTotalQtd + Produto1.FieldByName('MES_QUANTIDADE').asFloat;
    SZRLabel18.Caption:=format('%.2f',[(Produto1.FieldByName('MES_SALDO_ANT').asFloat+Produto1.FieldByName('MES_QUANTIDADE').asFloat)])
  end
  else
  Begin
    iTotalQtd := iTotalQtd - Produto1.FieldByName('MES_QUANTIDADE').asFloat;
...
procedure Trpt_MovEstoque.ZRBand1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  ZRLabel3.Caption:=DM.Configuracao1.LojaNome;
end;

procedure Trpt_MovEstoque.SZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
...
procedure Trpt_MovEstoque.ZRGroup1BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  with Produto1 do
...
procedure Trpt_MovEstoque.zrbSumarioBeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
  zrlTotalQtd.Caption := format('%.2f',[iTotalQtd]);
end;

procedure Trpt_MovEstoque.ZRBand3BeforePrint(Sender: TObject;
  var DoPrint: Boolean);
begin
