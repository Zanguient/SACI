...
//    procedure DrawAllSysbtn;
    ...
    function Checkvisible(i:integer):boolean;
...
begin
    ...
    if button.menuitem<>nil then RethinkLines(button.menuitem);
...
begin
  ...
  if message.result<>0 then exit;
    ...
    if (Button <> nil) and (ShiftState = [ssAlt]) then begin
      ...
      end else if (fform<>application.MainForm) then
          application.MainForm.Perform(CM_DIALOGCHAR,
                TWMKey(Message).CharCode,TWMKey(Message).KeyData);
      message.Result:=0;
      OldWndProc(message);
    end;  
end;
...
//   skinaddlog(aname);
   ...
   if (aname='button') and ((style and bs_ownerdraw)<>bs_ownerdraw) then begin
     ...
     if ((Style and BS_AUTOCHECKBOX )=BS_AUTOCHECKBOX)
         ...
         or ((Style and BS_3STATE )=BS_3STATE) then
//               c:=nil
             c := Tskincheckbox.create(Self)
      else if ((Style and BS_AUTORADIOBUTTON)=BS_AUTORADIOBUTTON)
             or ((Style and BS_RADIOBUTTON)=BS_RADIOBUTTON) then
             c := Tskinradiobutton.create(Self)
      else  c :=  Tskinbutton.create(Self);
   end
   else if (aname='combobox') then begin
       if ((Style and $03)<>CBS_SIMPLE) then
        c := TSkinCombox.create(Self) ;
   end
   else if (aname='comboboxex32') then begin
...
//        c := TSkinScrollbarH.create(Self) ;
   ...
   else if (aname='tcheckbox') then begin
         c := Tskincheckbox.create(Self);
   end
   else if (aname='tcombobox') then begin
         c := TSkinCombox.create(Self);
   end
   else if (aname='tbutton') then begin
...
begin
    if deficon<>0 then begin
       SmallIcon := CopyImage(DefIcon, IMAGE_ICON, iconx, iconx, LR_COPYFROMRESOURCE);
...
//    fcanvas.CopyRect(sysbtn[i].rect,sysbtn[i].data.map.canvas,r);
//    DrawRect1(FWorkDC,btn.bounds,btn.data.map,i,6);
    if btn.data.action in [3,11] then
//        DrawRect1(fworkdc,sysbtn[i].bounds,iconbmp,1,1,1)
        DrawIcon(fworkdc, btn.bounds)
//        DrawRect1(FWorkDC,btn.bounds,iconbmp,1,1,1)
    else
...
begin
 {$IFNDEF demo}
    if bidileft then
      DrawStyle := DT_END_ELLIPSIS or DT_SINGLELINE or DT_Right
    else
      DrawStyle := DT_END_ELLIPSIS or DT_SINGLELINE or DT_LEFT;
    ws:=' ';
...
//                ,r1, DT_CALCRECT or DT_NOCLIP);
       ...
       if bidileft then
          x:=(rc.right-rc.left-fsd.title.r.right-(r1.right-r1.left)) ;
       y:= ((rc.Bottom-rc.Top)-(r1.Bottom-r1.Top)) div 2 +
             fsd.title.r.top div 2;
       offsetrect(r1,x,y);
       if (dwstyle and ws_popup) = 0 then  begin
          if r1.right>rc.right-fsd.title.r.right then
               r1.right:=rc.right-fsd.title.r.right;
       end else begin
          if (r1.right>rightbtn-5) and (rightbtn>5) then
               r1.right:=rightbtn-5;
       end;
...
begin
  for i:= 0 to High(sysbtn) do
     if checkvisible(i) and (not sysbtn[i].data.map.empty)
...
//      and (sysbtn[i].data.CombineOp=-1) then begin
        ...
        if sysbtn[i].data.action in [3,11] then begin
          ...
          //DrawRect1(acanvas.handle,sysbtn[i].bounds,iconbmp,1,1,1)
        end else
...
function TWinSkinForm.checkvisible(i:integer):boolean;
var b:boolean;
begin
...
//               result:=false;
     ...
     6:result:= (sbiMax in formIcons) and (sbisystem in formicons);
     7:result:= not (sbiMax in formIcons);
     8:result:= (sbiMin  in formIcons) and (sbisystem in formicons);
     9:result:= not (sbiMin in formIcons);
     10,11: result :=false;
     100 : result:= MDIChildMax and (menu<>nil);
//     10:result:= biHelp in BorderIcons;
...
{     100 :
  ...
  case sysbtn[i].data.Visibility1 of
    -1 : b:=false;
     ...
     6:b:= sbiMax in formIcons;
     7:b:= not (sbiMax in formIcons);
     8:b:= sbiMin  in formIcons;
...
//   DrawStyle := DT_EXPANDTABS or DT_SINGLELINE or Alignments[Alignment];
   DrawStyle := DT_EXPANDTABS or DT_SINGLELINE or DT_RIGHT;
   DrawStyle2 := DT_Left;
...
var i:integer;
    r1:Trect;
begin
    if (fsd.button=nil) or (skinstate<>Skin_Active) then exit;
